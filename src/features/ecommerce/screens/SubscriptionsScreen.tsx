...
function SubscriptionCard({
    ...
    return (
        <TouchableOpacity
            activeOpacity={0.85}
            onPress={onSelect}
            className={`mb-4 overflow-hidden rounded-2xl bg-white p-4 border ${isSelected ? "border-[#1B5E37] border-2" : "border-gray-100"
                }`}
            style={{
                shadowColor: "#000",
        ...
        >
            <View className="mb-3 flex-row items-center justify-between">
                ...
                <View
                    className={`rounded-full px-2.5 py-1 ${isPaused ? "bg-amber-100" : "bg-green-100"
                        }`}
                >
                    <Text
                        className={`text-[10px] font-bold ${isPaused ? "text-amber-700" : "text-green-700"
                            }`}
                    >
